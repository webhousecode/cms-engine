/**
 * F125 reference reader for @webhouse/cms file-based content (Astro / TypeScript).
 *
 * Uses Node fs/path APIs — runs in Astro's SSR context. Designed to be thin
 * and safe — slugs and collection names are validated to prevent path traversal.
 */
...
function validate(name: string): void {
  if (!name || !SAFE_NAME.test(name)) {
    throw new InvalidName(`Invalid name '${name}' — must match ${SAFE_NAME}`);
  }
}
...
// Singleton — Astro's SSR context shares this across requests
